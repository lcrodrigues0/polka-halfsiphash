...
#columns(1)[
  ...
  + PolKA and the proposed extension are open source, so it is assumed that any attacker can replicate a node perfectly.
  + Protocol boundary is IPv4. This means that PolKA is only used inside this network. // Continue
  ...
  $ limits(P)_(e_1->e_10) = (e_1, #range(1,11).map(i => $s_#i$).join($,$), e_10) $
